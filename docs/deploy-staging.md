...
`CRON_SECRET`을 난수로 채우고, Vercel Cron에 `/api/cron/storage-cleanup`을 등록한다
(`vercel.json`의 `crons`, 하루 1회면 충분 — 고아는 쌓일 뿐 새지 않는다). Vercel은 등록된
cron 호출에 `Authorization: Bearer ${CRON_SECRET}`을 자동으로 싣는다. 등록 전까지는 같은
헤더로 curl 해서 수동으로 돌릴 수 있다. 시크릿 미설정이면 라우트가 전부 401이다.
